...
class PreextTask(Enum):
    ...
    ZOOM_IN = 8
    # ZOOM_OUT = 10,
    # FLIP_BOTH = 7 # BOTH horizontal and vertical, same as 180
...
hyperparameters = {
    ...
    'kernel_initializer': tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.05), # CHANGE 3,
    ...
    'initial_learning_rate': 0.001,  # Modified: Higher initial learning rate
...
def display_configurations():
    ...
    # Training Configuration
    training_keys = ['batch_size', 'epochs', 'validation_split', 'initial_learning_rate',
...
class CustomLearningRateScheduler(tf.keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        # Calculate new learning rate (10x slower at each epoch)
        new_lr = self.initial_learning_rate * (0.1 ** epoch)
        tf.keras.backend.set_value(self.model.optimizer.learning_rate, new_lr)
        print(f'\nEpoch {epoch+1}: Learning rate set to {new_lr}')

def build_basic_cnn(inputs):
    """
    Build a basic CNN model with Gaussian weight initialization
    """
    x = layers.Conv2D(
        hyperparameters['cnn_filters'][0], 
        hyperparameters['cnn_kernel_size'],
    ...
    x = layers.Dense(
        hyperparameters['cnn_dense_units'],
        activation=hyperparameters['cnn_activation_function'],
        kernel_initializer=hyperparameters['kernel_initializer']
    )(x)
...
def build_custom_model(input_shape=(128, 128, 1), num_classes=4, architecture_name='cnn', fine_tune=True):
    ...
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=hyperparameters['initial_learning_rate']),
        # optimizer=tf.keras.optimizers.SGD(learning_rate=hyperparameters['initial_learning_rate']), # CHANGE 2
...
def run_pretext_pipeline(x_augmented, y_augmented, architecture_name, model_path):
    ...
    callbacks = [
        # CustomLearningRateScheduler(hyperparameters['initial_learning_rate']),
...
def evaluate_downstream_task(clf, X_test, y_test, pretext_classifier, save_dir=FIGURES_DIR):
    ...
    if is_binary:
        ...
        for threshold in fpr_thresholds:
            # Find the closest FPR value to our threshold
    ...
    try:
        ...
        if mode in ['downstream', 'all']:
            ...
            for arch in architectures:
                ...
                # CHANGE 4
                # Reshape if needed
                # original_shape_train_features = train_features.shape
                # original_shape_test_features = test_features.shape

                # # Reshape to 2D if features are multi-dimensional
                ...
                # train_features_normalized = feature_scaler.fit_transform(train_features_2d)
                # test_features_normalized = feature_scaler.transform(test_features_2d)

                # # Reshape back to original dimensions if needed
                # train_features = train_features_normalized.reshape(original_shape_train_features)
                # test_features = test_features_normalized.reshape(original_shape_test_features)

                # # Verify normalization
                # print("Training features range after normalisation:", train_features.min(), "to", train_features.max())
                # print("Test features range after normalisation:", test_features.min(), "to", test_features.max())
